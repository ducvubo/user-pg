...
export default async function SearchRestaurant() {
  ...
  return (
    <div className=' mt-10 h-auto'>
      <div className='flex justify-between items-center'>
        <div className='flex flex-col gap-1'>
          <Label className='font-semibold text-xl sm:text-2xl md:text-3xl'>Tìm kiếm nâng cao</Label>
          <Label className='text-xs sm:text-sm md:text-base'>Tìm kiếm nhà hàng phù hợp với nhu cầu của bạn</Label>
        </div>
      </div>
      <hr className='my-3' />

      <div className='w-full rounded-xl bg-[#E85D4C] p-4 sm:p-6'>
        <div className='flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:gap-6'>
          <Select>
            <SelectTrigger className='h-10 !rounded-3xl w-full bg-[#E85D4C] text-white border-white/20 focus:ring-white'>
              <SelectValue placeholder='Tỉnh / Thành phố' />
            </SelectTrigger>
            <SelectContent>
              {listProvince.data.map((province: any) => (
          ...
          <Input
            placeholder='Nhập địa chỉ...'
            className='h-10 !rounded-3xl w-full bg-[#E85D4C] text-white border-white/20 focus-visible:border-white focus-visible:ring-0 placeholder:text-white'
          />
          ...
          <Select>
            ...
            <SelectContent>{/* Thêm danh sách loại hình ẩm thực nếu cần */}</SelectContent>
          </Select>
          ...
          <Select>
            ...
            <SelectContent>{/* Thêm danh sách loại hình phục vụ nếu cần */}</SelectContent>
          </Select>

          <div className='w-full md:w-auto'>
            <Button
              variant='outline'
              className='w-full h-10 text-red-500 font-semibold hover:text-red-500 rounded-3xl flex items-center justify-center gap-2'
            >
